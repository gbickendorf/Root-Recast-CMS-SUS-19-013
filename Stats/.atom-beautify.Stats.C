...
void HistFactoryModel(double m_Go)
{
   ...
   signalOutFile << "SIGNALS/"<<"Signal137fbGluino"<<m_Go<<".txt";
   ...
   f2.Close();
      ...
      string line;
      ifstream myfile (signalOutFile.str().c_str());
      if (myfile.is_open())
      {
        while ( getline (myfile,line) )
        {
          vsignal.push_back(stod(line));
          vsignalErr.push_back(sqrt(stod(line)));
         }
        myfile.close();
      }

   cout << transferFac << "  " << transferFacErr << endl;
   for (size_t i = 0; i < 6; i++) {
     cout << vdata[i] << " " << vsignal[i] << " " << vCRi[i]*transferFac << " " << vCRi[i] << endl;
   }
   ...
   signal.AddNormFactor("mu", 1, 0, 60);
   signal.AddOverallSys("signalTheoryConstraint", TheoryXSectionLower(m_Go) / TheoryXSection(m_Go), TheoryXSectionUpper(m_Go) / TheoryXSection(m_Go));
   ...
   backgSR.AddNormFactor("TransferFac", transferFac, max(0.0, transferFac - 10 * transferFacErr), transferFac + 10 * transferFacErr);
   // meas.AddConstantParam("TransferFac");
   backgSR.AddOverallSys("transFacConstraint",max(0.01, 1 - transferFacErr / transferFac), 1 + transferFacErr / transferFac);
   backgSR.SetHisto(h1_bSR);
   // shapeSys_S.SetErrorHist(hShapeSyst);
   // backgSR.AddShapeSys(shapeSys_S);
   /*
   The computed upper limit is: 0.069382 +/- 0
Expected upper limits, using the B (alternate) model :
 expected limit (median) 0.0396301
 expected limit (-1 sig) 0.026493
 expected limit (+1 sig) 0.0610285
 expected limit (-2 sig) 0.0191383
 expected limit (+2 sig) 0.0926065
*/
...
void BrazilianPlot(vector<double> m_Go, vector<vector<double>> limits)
{
  ...
  double fontsize = 0.07;
   auto c41 = new TCanvas("c41", "c41", 20, 10, 5000*scale, 3000*scale);
   gPad->SetTickx();
    gPad->SetTicky();
    c41->SetRightMargin(0.0);
   ...
   ge->SetTitle("");
   ge->GetXaxis()->SetTitleSize(fontsize);
   ge->GetXaxis()->SetLabelSize(fontsize);
   ge->GetXaxis()->SetTitle("m(#tilde{g}) [GeV]");
   ge->GetYaxis()->SetTitleSize(fontsize);
   ge->GetYaxis()->SetLabelSize(fontsize);
   ge->GetYaxis()->SetTitle("Corss section [pb]");
   ge->GetYaxis()->SetTitleOffset(1);
   ...
   auto legend = new TLegend(0.6,0.5,0.99,0.85);
   ...
   gPad->SetTicky();
   gPad->RedrawAxis();
   c41->SaveAs("../Plots/Crosssections137.pdf");
}
...
void RunStatsMacro()
{
   ...
   TFile *fout = TFile::Open("StatsOutput137.root", "RECREATE");
...
void DoPlots()
{
   TFile *fin = TFile::Open("StatsOutput137.root", "READ");
   vector<vector<double>> limits;
   vector<double> m_Go;
